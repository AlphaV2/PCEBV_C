...
- `App.tsx` - top-level page composition, route selection, modal state, language wiring, WhatsApp link generation
...
- `src/components/Navbar.tsx` - desktop/mobile navigation, route-aware dropdowns, and language switcher
...
- `src/pages/ServicesPage.tsx` - dedicated services page with detailed classifications
- `src/pages/ProjectsPage.tsx` - dedicated projects page wrapper and overview
- `src/pages/AboutPage.tsx` - dedicated about page with founder profiles and company story
- `src/pages/ContactPage.tsx` - dedicated contact page with strategy, form, and office cards
...
1. `App.tsx` initializes the page, selects the active route, and passes modal handlers into child components.
2. `Navbar.tsx` renders translated navigation, route-aware dropdowns, and mobile controls.
3. `useTranslatedData.ts` converts base data/constants into translated view models.
4. Home sections render preview content only, while `/services`, `/projects`, `/about`, and `/contact` render the detailed route pages.
...
## 9. Route And Navigation Model

The site now uses route-based navigation for the main information pages.

Current paths:
- `/` - homepage with compact preview sections
- `/services` - detailed service classifications and scope breakdowns
- `/projects` - dedicated project experience page
- `/about` - company story, founders, and operating model
- `/contact` - contact strategy, form, and office cards

Navigation behavior:
- Home links navigate to `/` and scroll to top when already on the homepage.
- Services, Projects, About, and Contact open dedicated pages rather than dumping all content on the home page.
- Dropdown cards are intentionally compact so they stay readable on smaller screens.
- Route links with hashes are used for page-local anchors such as `/about#founders` and `/contact#form`.

Implementation notes:
- Keep page navigation in `Navbar.tsx` route-aware.
- Prefer `window.location.assign(...)` for cross-page navigation in this no-router setup.
- Keep homepage content as a teaser layer, not the canonical full description.

## 10. Content And Modal Behavior
...
- split homepage content into dedicated route pages
- compact route-aware dropdown navigation in the header
