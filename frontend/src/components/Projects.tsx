...
const Projects: React.FC = () => {
  ...
  const resolveImageSrc = (img?: string) => {
    ...
    if (img.startsWith('/')) return img; // root-relative path
    // otherwise assume it's in /public/projects/
    return `/projects/${img}`;
  };
    ...
    return (
      <section id="projects" className="py-20 bg-slate-50 text-center">
        ...
        <p className="text-slate-500 font-medium mt-2">
          {t('projects.loadingDeployments', 'Loading Deployments...')}
        </p>
      </section>
    );
  }
            ...
            >
              ...
              <div className="h-32 sm:h-36 overflow-hidden relative shrink-0">
                ...
                <div className="absolute top-2 left-2 bg-white/95 backdrop-blur-md px-1.5 py-0.5 text-[9px] font-bold rounded-md uppercase tracking-wider text-slate-900 border border-slate-200 shadow-sm z-10 flex items-center gap-1">
                  {project.category?.includes('Drone') ? (
                  ...
                  <div className="mt-auto pt-2 border-t border-slate-100">
                    <div className="grid grid-cols-2 gap-1">
                      {project.metrics.slice(0, 2).map((metric: string, idx: number) => (
                        <div
                          key={idx}
                          className="bg-slate-50 px-1 py-1 rounded-md border border-slate-100 text-center"
                        >
                          <div className="text-[8px] uppercase font-bold text-slate-400 mb-0.5 tracking-wider">
                            {t('projects.metric', 'Metric')}
                          </div>
