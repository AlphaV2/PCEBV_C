...
import { useTranslatedGallery } from '../hooks/useTranslatedData';
...
const MobileSwipeCard: React.FC<{ item: any; onClick: () => void; priority?: boolean }> = memo(({ item, onClick, priority = false }) => {
  const isPamphlet = item.id?.toString().includes('pamphlet');
  ...
  return (
    ...
    >
      ...
      <img
        ...
        decoding="async"
        className={`absolute inset-0 w-full h-full transition-all duration-500 z-10 ${isPamphlet ? 'object-contain p-2' : 'object-cover'}`}
  ...
  return (
    ...
    >
      ...
      <img
        ...
        className={`absolute inset-0 w-full h-full transition-all duration-700 ease-out z-10 ${objectFit === 'contain' ? 'object-contain bg-slate-950 p-1 md:p-4' : 'object-cover'} ${isImageLoaded ? 'opacity-100 scale-100' : 'opacity-0 scale-105 blur-sm'} ${objectFit === 'cover' ? 'group-hover:scale-105' : ''}`}
        sizes="(max-width: 768px) 100vw, 33vw"
        onError={(e) => (e.currentTarget.src = 'https://placehold.co/600x400?text=No+Image')}
      />
      {objectFit === 'cover' && <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent z-10 opacity-90 pointer-events-none" />}
      ...
      <div className="absolute bottom-0 left-0 w-full p-4 z-20 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
        ...
        {currentItem.description && <p className="text-[10px] md:text-xs text-slate-300 mt-1 line-clamp-2 leading-relaxed opacity-80">{currentItem.description}</p>}
      </div>
      ...
      {showOfferBadge && (
        <div className="absolute top-3 right-3 bg-red-500 text-slate-950 text-[10px] font-black px-2.5 py-1 rounded shadow-lg z-30 animate-pulse pointer-events-none tracking-wider border border-yellow-300 flex items-center gap-1">
          <Zap size={10} fill="currentColor" /> {t('gallery.offerBadge', 'OFFER')}
        </div>
      )}
      ...
      {items.length > 1 && (
        <div className="absolute top-3 left-3 flex gap-1 z-20 pointer-events-none">
          {items.map((_, idx) => (
            <span key={idx} className={`h-1 rounded-full shadow-sm transition-all duration-500 ${idx === currentIndex ? 'w-5 bg-blue-500' : 'w-1.5 bg-white/30'}`} />
          ))}
        </div>
      )}
    </div>
...
const Gallery: React.FC = () => {
  ...
  const content = useTranslatedGallery() as any;
  ...
  const pamphletContent = content?.pamphlet || [
    { id: 'def', src: '/gallery/pamphlet.jpeg', title: t('gallery.fallback.fullSpecifications', 'Full Specifications'), tagline: t('gallery.fallback.brochure', 'Brochure') },
  ];

  const mobileGalleryItems = [...pamphletContent, ...specialOffers, ...droneFleet, ...verticalBanners, ...innovations, ...exhibitions];

  useEffect(() => {
    if (window.innerWidth < 768 || isSectionHovered || lightboxState) return;
    const duration = activeSectionSlide === 0 ? SLIDE_DURATION_PAMPHLET : SLIDE_DURATION_GRID;
    const timer = setTimeout(() => setActiveSectionSlide((prev) => (prev === 0 ? 1 : 0)), duration);
    return () => clearTimeout(timer);
  }, [activeSectionSlide, isSectionHovered, lightboxState]);

  const openLightbox = useCallback((items: any[], index: number) => setLightboxState({ items, index }), []);
  ...
  const nextSection = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    setActiveSectionSlide((prev) => (prev === 0 ? 1 : 0));
  }, []);
  const prevSection = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    setActiveSectionSlide((prev) => (prev === 0 ? 1 : 0));
  }, []);
  ...
  return (
    <section id="gallery" className="py-10 md:py-16 bg-slate-950 text-white scroll-mt-20 overflow-hidden border-t border-slate-900" onMouseEnter={() => setIsSectionHovered(true)} onMouseLeave={() => setIsSectionHovered(false)}>
      <div className="container mx-auto px-0 md:px-6 lg:px-8">
        <div className="px-4 md:px-0 mb-4 md:mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-3 border-b border-slate-800/50 pb-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 mb-2 text-[10px] font-bold tracking-widest text-blue-400 uppercase bg-blue-950/30 rounded-full border border-blue-900/50">
              <span className={`w-1.5 h-1.5 rounded-full ${activeSectionSlide === 0 ? 'bg-green-500' : 'bg-blue-500'} animate-pulse`} />
              {activeSectionSlide === 0 ? t('gallery.sectionFeaturedSpotlight', 'Featured Spotlight') : t('gallery.sectionOperationalGrid', 'Operational Grid')}
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight">{t('gallery.title', 'Drone Fleet & Operations Gallery')}</h2>
          </div>

          <div className="hidden md:flex gap-2">
            <button onClick={() => setActiveSectionSlide(0)} className={`h-1.5 rounded-full transition-all duration-500 ${activeSectionSlide === 0 ? 'w-8 bg-blue-500' : 'w-2 bg-slate-700 hover:bg-slate-600'}`} />
            <button onClick={() => setActiveSectionSlide(1)} className={`h-1.5 rounded-full transition-all duration-500 ${activeSectionSlide === 1 ? 'w-8 bg-blue-500' : 'w-2 bg-slate-700 hover:bg-slate-600'}`} />
          </div>
        </div>
        ...
        <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-8 no-scrollbar px-4 pt-2">
          {mobileGalleryItems.map((item, index) => (
            <MobileSwipeCard key={item.id || index} item={item} priority={index === 0} onClick={() => openLightbox(mobileGalleryItems, index)} />
          ))}
        </div>
            ...
            <div className={`absolute inset-0 w-full h-full transition-opacity duration-700 ease-in-out ${activeSectionSlide === 1 ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 h-full">
                <div className="h-full w-full">
                  <CarouselCard items={verticalBanners} intervalTime={3000} onOpenLightbox={(idx) => openLightbox(verticalBanners, idx)} className="h-full" autoPlay={activeSectionSlide === 1} />
                </div>
                <div className="flex flex-col gap-4 h-full">
                  <div className="flex-1 relative">
                    <CarouselCard items={specialOffers} intervalTime={4500} onOpenLightbox={(idx) => openLightbox(specialOffers, idx)} showOfferBadge className="h-full" autoPlay={activeSectionSlide === 1} />
                  </div>
                  <div className="flex-1 relative">
                    <CarouselCard items={droneFleet} intervalTime={3500} onOpenLightbox={(idx) => openLightbox(droneFleet, idx)} className="h-full" autoPlay={activeSectionSlide === 1} />
                  </div>
                </div>
                <div className="flex flex-col gap-4 h-full">
                  <div className="flex-1 relative">
                    <CarouselCard items={innovations} intervalTime={3000} onOpenLightbox={(idx) => openLightbox(innovations, idx)} className="h-full" autoPlay={activeSectionSlide === 1} />
                  </div>
                  <div className="flex-1 relative">
                    <CarouselCard items={exhibitions} intervalTime={3000} onOpenLightbox={(idx) => openLightbox(exhibitions, idx)} className="h-full" autoPlay={activeSectionSlide === 1} />
                  </div>
                </div>
              </div>
            </div>
      ...
      {lightboxState && (
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={closeLightbox}>
          <button className="absolute top-4 right-4 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-all z-50 backdrop-blur-md" onClick={closeLightbox}>
            <X size={24} />
          </button>

          <div className="relative w-full max-w-5xl max-h-[90vh] bg-black rounded-xl overflow-hidden shadow-2xl border border-slate-800 flex flex-col md:flex-row" onClick={(e) => e.stopPropagation()}>
            <div className="relative flex-1 bg-black flex items-center justify-center p-4 group select-none">
              <img src={lightboxState.items[lightboxState.index].src} alt={t('gallery.lightboxImageAlt', 'View')} className="max-w-full max-h-[50vh] md:max-h-[80vh] object-contain" />
            ...
            <div className="bg-slate-900/90 backdrop-blur-md p-6 border-t md:border-t-0 md:border-l border-slate-800 w-full md:w-96 flex flex-col shrink-0">
              <div className="flex-1">
                ...
                {lightboxState.items[lightboxState.index].tagline && <div className="inline-block px-2 py-0.5 mb-4 text-[10px] font-bold tracking-widest text-blue-300 uppercase bg-blue-900/30 rounded border border-blue-800">{lightboxState.items[lightboxState.index].tagline}</div>}
                <p className="text-slate-400 text-sm leading-relaxed mb-6">{lightboxState.items[lightboxState.index].description || t('gallery.lightboxDefaultDescription', 'Operational view captured during recent field deployment.')}</p>
                <button onClick={() => handleWhatsAppClick(lightboxState.items[lightboxState.index])} className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white py-3 px-4 rounded-lg font-bold transition-all shadow-lg hover:shadow-green-900/50 group">
                  <MessageCircle size={20} className="group-hover:scale-110 transition-transform" />
                  {t('gallery.inquireOnWhatsapp', 'Inquire on WhatsApp')}
                </button>
              </div>
              <div className="mt-6 pt-4 border-t border-slate-800">
                {lightboxState.items[lightboxState.index].location && <p className="text-sm text-slate-300 flex items-center gap-2 mb-2"><MapPin size={14} className="text-blue-500" /> {lightboxState.items[lightboxState.index].location}</p>}
                <div className="text-xs text-slate-500 font-mono text-right">{t('gallery.imageCounter', 'Image')} {lightboxState.index + 1} {t('gallery.imageCounterOf', 'of')} {lightboxState.items.length}</div>
              </div>
            </div>
          </div>
        </div>
